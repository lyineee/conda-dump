...
def dump(package:str, exclude_list:List[str]):
    conda_env_name = (lambda: "base" if env_name == "" else os.path.split(env_name)[-1])()
    print(f'dump package "{package}" in envirment "{conda_env_name}", exclude_list: {exclude_list}')
    print(f'dump destination path: "{dump_path}"')
    req = DependReq(name=package)
    dep_info = search_dep(req)
    if not dep_info:
        print(f'ERROR: package "{package}" not found in conda envirment "{conda_env_name}", dump stop')
        sys.exit(1)
    ex = []
...
def init_args():
    ...
    parser.add_argument('-n','--conda-env', type=str, dest="conda_env", help="specify a envirment name")
    parser.add_argument('-c', '--conda-path', type=str,dest="conda_path", help="specify a conda base path")
    parser.add_argument('-e', '--exclude', action="append",type=str, help="exclude exist package")
    parser.add_argument('-d','--dump-path',dest="dump_path", help="dump destination directory")
    parser.add_argument('-t', '--print-tree',action="store_true",dest="print_tree", help="print dependent tree")
    ...
    if not os.path.exists(conda_base):
        print(f'conda path not found in default path {conda_base}, please specify another')
        sys.exit(1)
    if args.conda_env:
